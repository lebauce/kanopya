...
throws_ok { my $groups = Entity::Groups->get(id => 1) } "Kanopya::Exception::AuthenticationRequired", 'Authentication required to retrieve an Entity::Groups';
...
lives_ok { Administrator::authenticate(login => 'admin', password => 'admin'); }  "Authentication succeed with correct login/password for admin user";
lives_ok { Administrator::authenticate(login => 'executer', password => 'executer'); }  "Authentication succeed with correct login/password for executer user";
...
Administrator::authenticate(login => 'executer', password => 'executer');
my $user_executer_eid = $ENV{EID};
...
$adm = Administrator->new();
isnt($ENV{EID}, $user_executer_eid, "environment variable EID changed during reauthentication");
