...
my $install_conf = XMLin("C:/opt/kanopya/scripts/install/init_struct.xml");
...
sub genConf {
    ...
    foreach my $files (keys %$conf_files){
		...
		print Dumper($conf_files->{$files}->{template});
		print Dumper(\%datas);
		print Dumper($conf_vars->{conf_dir} . $files);
		print Dumper($conf_vars->{install_template_dir});
        useTemplate(
...
sub useTemplate{
	...
	if (not defined $template) {print 'output file: '.$output;}
