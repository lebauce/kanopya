...
mysql -u root -p$password < /opt/kanopya/scripts/database/mysql/schemas/Schemas.sql
echo 'done'
echo -n 'insert initial data... '
mysql -u root -p$password < /opt/kanopya/scripts/database/mysql/data-sample/initialize_for_Bench_client.sql 
echo 'done'
echo '> WARNING ! <'
