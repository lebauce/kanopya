...
sub generate_nfs_common {
    ...
    $template->process($input, $data, "/tmp/".$tmpfile) || do {
        $errmsg = "EComponent::ENfsd3->generate_nfs_common : error during template generation : $template->error;";
        $log->error($errmsg);
...
sub generate_nfs_kernel_server {
    ...
    $template->process($input, $data, "/tmp/".$tmpfile) || do {
        $errmsg = "EComponent::ENfsd3->generate_nfs_kernel_server : error during template generation : $template->error;";
        $log->error($errmsg);
        throw Kanopya::Exception::Internal(error => $errmsg);    
    };
...
sub generate_exports {
    ...
    $template->process($input, $data, "/tmp/".$tmpfile) || do {
        $errmsg = "EComponent::ENfsd3->generate_exports : error during template generation : $template->error;";
        $log->error($errmsg);
        throw Kanopya::Exception::Internal(error => $errmsg);    
    };
