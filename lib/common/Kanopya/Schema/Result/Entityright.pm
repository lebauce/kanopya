use utf8;
package Kanopya::Schema::Result::Entityright;

# Created by DBIx::Class::Schema::Loader
# DO NOT MODIFY THE FIRST PART OF THIS FILE

=head1 NAME

Kanopya::Schema::Result::Entityright

=cut

use strict;
use warnings;

=head1 BASE CLASS: L<DBIx::Class::IntrospectableM2M>

=cut

use base 'DBIx::Class::IntrospectableM2M';

=head1 LEFT BASE CLASSES

=over 4

=item * L<DBIx::Class::Core>

=back

=cut

use base qw/DBIx::Class::Core/;

=head1 TABLE: C<entityright>

=cut

__PACKAGE__->table("entityright");

=head1 ACCESSORS

=head2 entityright_id

  data_type: 'integer'
  extra: {unsigned => 1}
  is_auto_increment: 1
  is_nullable: 0

=head2 entityright_consumed_id

  data_type: 'integer'
  extra: {unsigned => 1}
  is_foreign_key: 1
  is_nullable: 0

=head2 entityright_consumer_id

  data_type: 'integer'
  extra: {unsigned => 1}
  is_foreign_key: 1
  is_nullable: 0

=head2 entityright_method

  data_type: 'char'
  is_nullable: 0
  size: 64

=cut

__PACKAGE__->add_columns(
  "entityright_id",
  {
    data_type => "integer",
    extra => { unsigned => 1 },
    is_auto_increment => 1,
    is_nullable => 0,
  },
  "entityright_consumed_id",
  {
    data_type => "integer",
    extra => { unsigned => 1 },
    is_foreign_key => 1,
    is_nullable => 0,
  },
  "entityright_consumer_id",
  {
    data_type => "integer",
    extra => { unsigned => 1 },
    is_foreign_key => 1,
    is_nullable => 0,
  },
  "entityright_method",
  { data_type => "char", is_nullable => 0, size => 64 },
);

=head1 PRIMARY KEY

=over 4

=item * L</entityright_id>

=back

=cut

__PACKAGE__->set_primary_key("entityright_id");

=head1 UNIQUE CONSTRAINTS

=head2 C<entityright_consumed_id>

=over 4

=item * L</entityright_consumed_id>

=item * L</entityright_consumer_id>

=item * L</entityright_method>

=back

=cut

__PACKAGE__->add_unique_constraint(
  "entityright_consumed_id",
  [
    "entityright_consumed_id",
    "entityright_consumer_id",
    "entityright_method",
  ],
);

=head1 RELATIONS

=head2 entityright_consumed

Type: belongs_to

Related object: L<Kanopya::Schema::Result::Entity>

=cut

__PACKAGE__->belongs_to(
  "entityright_consumed",
  "Kanopya::Schema::Result::Entity",
  { entity_id => "entityright_consumed_id" },
  { is_deferrable => 1, on_delete => "CASCADE", on_update => "NO ACTION" },
);

=head2 entityright_consumer

Type: belongs_to

Related object: L<Kanopya::Schema::Result::Entity>

=cut

__PACKAGE__->belongs_to(
  "entityright_consumer",
  "Kanopya::Schema::Result::Entity",
  { entity_id => "entityright_consumer_id" },
  { is_deferrable => 1, on_delete => "CASCADE", on_update => "NO ACTION" },
);


# Created by DBIx::Class::Schema::Loader v0.07033 @ 2013-11-20 15:15:44
# DO NOT MODIFY THIS OR ANYTHING ABOVE! md5sum:2Od5LB6u1dybbt0PGFcdug


# You can replace this text with custom code or comments, and it will be preserved on regeneration
1;
